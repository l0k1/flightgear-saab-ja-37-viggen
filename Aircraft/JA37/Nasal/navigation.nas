...
var nav = "/sim/ja37/navigation";

datpan = {
	nav:					nav,
	dp_mode:				nav ~ "/dp-mode",
	dp_display:				nav ~ "/dp-display",
	dp_display_pos:			nav ~ "/dp-display-pos",
	dp_display_readout:		nav ~ "/dp-display-readout",
	inout:					nav ~ "/inout", # 0 = in
	ispos:					nav ~ "/ispos",
	tils_knob:				nav ~ "/tils-knob",
	tils_group:  			nav ~ "/tils-group",
	dp_prop_input:			nav ~ "/dp-display-input",
	np_last_pressed:		nav ~ "/np-last-pressed",
	idnr:					nav ~ "/idnr",
};
...
############################################################
### Datapanel/Navpanel Hardware Functions
############################################################
...
# This function takes the property dp_prop_input and displays what is there on the datapanel.
# To update display to new value immediately, and destroy prior loops, increment display_id and call this function with the new ID.
var display_driver = func (id,loop_time) {
	id == display_id or return;
	clear_display();
	var display_datum = datpan.dp_prop_input.getValue();
	var divisor = 1000000;
	if ( display_datum == 0 or display_datum == nil ) { return; }
	...
	if (loop_time > 0) {
		settimer( func { display_driver(id,loop_time); }, loop_time);
	}
}
...
########## DATAPANEL NUMPAD/RENSA

# If a button on the numpad on the datapanel portion is entered, run this function.
# Only run it if the switch is in the "IN" position, or we are in BX mode.
...
############################################################
### Datapanel/Navpanel Software Functions
############################################################
...
var computer_logic = func(id, loop_time) {
	
	id == logic_id or return;
	
	print("In computer_logic, past first return.");
	...
	if ( datpan.dp_mode.getValue() == 0 and datpan.inout.getValue() == 1 ) {
		print("inside act-pos");
		...
		# update every second
		display_driver(display_id,0);
		
		computer_logic(logic_id,1);
			
		
	########################### knob is at ref/lola
	# show lat/lon of current waypoint/bp button/landing button.
	} elsif ( datpan.dp_mode.getValue() == 1 and datpan.inout.getValue() == 1 ) {
	...
	} elsif ( datpan.dp_mode.getValue() == 2 and datpan.inout.getValue() == 1 ) {
	...
	} elsif ( datpan.dp_mode.getValue() == 3 and datpan.inout.getValue() == 1 ) {
		#output wind values
	...
	## if there's a timetable inputted, default to time table error
	## if not, estimated total flight time displayed default
	} elsif ( datpan.dp_mode.getValue() == 4 and datpan.inout.getValue() == 1 ) {
	...
	## shows remaining fuel in kilograms
	} elsif ( datpan.dp_mode.getValue() == 5 and datpan.inout.getValue() == 1 ) {
		...
		if ( key == 0 ) {
			settimer( func { computer_logic(); }, 1);
		} else {
			datpan.np_last_pressed.setValue(0);
		}
	
	########################### knob is at id-nr
	} elsif ( datpan.dp_mode.getValue() == 6 and datpan.inout.getValue() == 1 ) {
